...
export class CommonPageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      this.layoutComponent.setPageTitle("UN OPS | "+this.categoryName);
  ...
  getDynamicColumn(){
    ...
    this.sharedService.getDynamicColumn(jsonData)
    .subscribe((response) =>{
  ...
  viewDetails(event){
    ...
    this.submitLatlong = event.data.submitLatlong;
    let jsonData = {
      loginEmpId : this.loginEmpId,
  ...
  exportData(){
    if(this.transactionHdrList.length != 0){
      ...
      sql += 'INTO XLSXML("Report.xls",{headers:true}) FROM ?';
